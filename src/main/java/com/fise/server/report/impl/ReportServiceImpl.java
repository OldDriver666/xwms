...
public class ReportServiceImpl implements IReportService {
	...
	public Map<Integer,Integer> queryTypeDayMessages(String daytime) {
		...
		for (int i = 0; i < 8; i++) {
			String tablename = "IMMessage_";
			tablename += i;
			list = reportDao.queryTypeDayMessages(tablename, daytime);
			for (MessageTypeResult msg : list) {
				if (null == map.get(msg.getKeyName())) {
					map.put(msg.getKeyName(), msg.getKeyValue());
				} else {
					map.put(msg.getKeyName(), map.get(msg.getKeyName()) + msg.getKeyValue());
				}
			}
